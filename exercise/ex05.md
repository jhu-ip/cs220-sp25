...
1.	A subdirectory named *my220repo*, cloned from your personal private github repo. 
	...
	<div class='content'>
	<p>Remember in the previous exercise, we renamed the ugrad copy of your private repo from <em>2025-spring-student-JHED</em> to <em>my220repo</em> for brevity.</p>
	</div>
	</div>

2.	Another subdirectory *cs220-sp25-public*, cloned from the public, coursewide repo.

If this is not the case, ask for assistance.  You may need to redo steps from earlier exercises. 

### Part 2

1.	Type `cd cs220-sp25-public` to navigate into your working copy of the remote, coursewide public repo.
...
### Part 3
1.	We created a `.bashrc` unix shell file that contains several alias and environment settings that will make your ugrad server lives better. Along with this, we have a `.bash_profile` configuration file which will get the `.bashrc` settings into place when you log in remotely. While you are still in the *cs220-sp25-public* directory under `ex05`, type `ls -a` to see these (hidden) files.
...
5.	Copy the three template source files to the current directory using: `cp ~/cs220-sp25-public/exercises/ex05/*.c .` (Don't forget the space-then-dot at the end of the line above! Also, remember to insert any subdirectories you may have created into the path above.)
